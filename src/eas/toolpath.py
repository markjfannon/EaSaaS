...
def cross_x_search(point: tuple[int,int], img: np.ndarray) -> tuple[bool, tuple[int, int]]:
    """
...
def gen_line_command(direction: tuple[int,int], point: tuple[int,int], img: np.ndarray) -> tuple[Command, tuple[int, int],np.ndarray]:
    """
...
def find_next_point(img: np.ndarray, total_white: int, origin: tuple[int, int] = (0,0)) -> tuple[tuple[int, int], np.ndarray]:
